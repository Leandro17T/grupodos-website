...
if (!class_exists ('Updates_Estados_Pedido_Personalizados')) :
	...
	class Updates_Estados_Pedido_Personalizados {
		...
		public function show_update_notification($file, $plugin) {
			...
			if (is_network_admin() || !current_user_can ('update_plugins') || !is_multisite() || $this->name != $file)
				return;
			...
			if (!empty ($update_cache->response[$this->name]) && version_compare($this->version, $version_info->new_version, '<')) {
				...
				if (empty ($version_info->download_link))
					...
					printf(
						__('Hay disponible una nueva versión de %1$s. %2$sVer los detalles de la versión %3$s%4$s o %5$sactualizar ahora%6$s.', 'estados-pedido'),
						esc_html($version_info->name),
						'<a target="_blank" class="thickbox" href="' . esc_url($changelog_link) . '">',
						esc_html($version_info->new_version),
						'</a>',
		...
		public function show_changelog () {
			...
			if (!current_user_can ('update_plugins'))
				wp_die(__('No tienes permisos para instalar actualizaciones de plugins.', 'estados-pedido'), __('Error', 'easy-digital-downloads'), ['response' => 403]);	
