...
if ( ! class_exists( 'WCJ_SKU' ) ) :
	...
	class WCJ_SKU extends WCJ_Module {
		...
		public function generate_default_hashids_salt() {
			...
			isset( $_GET['section'] ) && 'sku' === $_GET['section']
				...
				if ( empty( wcj_get_option( 'wcj_sku_hashids_salt_default', '' ) ) ) {
					...
					$random_string = substr( str_shuffle( str_repeat( $x, ceil( $length / strlen( $x ) ) ) ), 1, $length );
		 ...
		 */
		public function get_hashids_lib() {
			if (
			! is_admin() ||
		 ...
		 */
		public function set_new_sku_on_save( $post_id ) {
		 ...
		 */
		public function remove_sku_from_admin_emails( $args ) {
		 ...
		 * @param array $args defines the args.
		 */
		public function add_sku_to_customer_emails( $args ) {
			if ( ! $args['sent_to_admin'] ) {
				$args['show_sku'] = true;
			}
			return $args;
		 ...
		 */
		public function get_all_variations( $_product ) {
		 ...
		 */
		public function get_sequential_counter( $product_id ) {
			...
			if ( $product->is_type( 'variable' ) ) {
				...
				if ( 'as_variable' === $variation_handling ) {
					...
					foreach ( $variations as $variation ) {
						...
						$variations_excluding_current = array_filter(
							$variations,
		 ...
		 * @since  1.0.0
		 *
		 * @param array $sku defines the sku.
		 * @param array $variations defines the variations.
		 *
		 * @return bool
		 */
		public function has_sku_been_used_in_variations( $sku, $variations ) {
			$variations_skus = wp_list_pluck( $variations, 'sku' );
			return in_array( $sku, $variations_skus, true );
		}
		 ...
		 * @param string         $str defines the str.
		 * @param array | string $sep defines the sep.
		 */
		public function get_acronym( $str, $sep ) {
			$acronym = '';
		 ...
		 * @param string $str defines the str.
		 * @param bool   $is_variation defines the is_variation.
		 */
		public function maybe_process_attributes( $_product, $format_template, $str, $is_variation = false ) {
			$pos     = strpos( $format_template, '{' . $str . '=' );
			$pos_end = strpos( $format_template, '}', $pos );
			while ( false !== ( $pos ) ) {
				if ( false !== ( $pos_end ) ) {
					if ( ! isset( $atts ) ) {
						if ( $is_variation ) {
		 ...
		 *
		 * @param int     $product_id defines the product_id.
		 * @param string  $variation_suffix defines the variation_suffix.
		 * @param int     $parent_product_id defines the parent_product_id.
		 * @param array   $_product defines the _product.
		 * @param boolean $get_sku_from_parent defines the get_sku_from_parent.
		 *
		 * @return mixed|string
		 */
		public function generate_sku( $product_id, $variation_suffix, $parent_product_id, $_product, $get_sku_from_parent = true ) {
			...
			$format_template = wcj_get_option(
				'wcj_sku_template',
			...
			$parent_sku      = get_post_meta( $parent_product_id, '_sku', true );
			$replace_values  = array(
				'{parent_sku}'                  => empty( $parent_sku ) ? $parent_product->get_sku() : $parent_sku,
		 ...
		 * @param int           $product_id defines the product_id.
		 * @param array         $sku_object defines the sku_object.
		 * @param string | bool $is_preview defines the is_preview.
		 */
		public function set_sku( $product_id, $sku_object, $is_preview ) {
			...
			if ( $is_preview ) {
				...
				update_post_meta( $product_id, '_sku', $the_sku );
			}
		}
		 ...
		 * @since  1.0.0
		 */
		public function maybe_get_sequential_counters() {
		 ...
		 * @param string | bool $is_preview defines the is_preview.
		 */
		public function maybe_save_sequential_counters( $is_preview = false ) {
		 ...
		 */
		public function set_new_product_sku_on_duplicate( $post_ID, $post ) {
			$this->maybe_get_sequential_counters();
			$this->set_sku_with_variable( $post_ID, false );
			$this->maybe_save_sequential_counters();
		}
		 ...
		 * @version 6.0.0
		 * @since   1.0.0
		 * @param string $join defines the join.
		 */
		 ...
		 * @version 6.0.0
		 * @since   1.0.0
		 * @param string  | array $query defines the query.
		 */
		public function add_search_by_sku_to_frontend( $query ) {
			if (
			isset( $query->query ) &&
			! $query->is_admin &&
			$query->is_search &&
