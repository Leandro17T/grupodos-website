 ...
 * @since  1.0.0
 * @author  Pluggabl LLC.
 * @package Booster_Plus_For_WooCommerce/shortcodes
 */
	...
	class WCJ_Products_Add_Form_Shortcodes extends WCJ_Shortcodes {
		...
		public function wc_add_new_product( $args, $shortcode_atts ) {
			...
			if ( 0 !== $product_id ) {
				...
				if ( isset( $args['image_gallery']['name'][0] ) && '' !== $args['image_gallery']['name'][0] ) {
					$files = isset( $_FILES['wcj_add_new_product_gallery_image'] ) ? $_FILES['wcj_add_new_product_gallery_image'] : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
					foreach ( $files['name'] as $key => $value ) {
						...
						foreach ( $_FILES as $file => $array ) {
							$error_file = sanitize_text_field( wp_unslash( isset( $_FILES[ $file ]['error'] ) ) );
							if ( UPLOAD_ERR_OK !== $error_file ) {
								__return_false();
							}
							require_once ABSPATH . 'wp-admin/includes/image.php';
							...
							@$gallery        .= $attachment_id . ','; // phpcs:ignore
							$i++;
						}
		...
		public function wcj_product_add_new( $atts ) {
			...
			$args = array(
				...
				'tags'          => isset( $_REQUEST['wcj_add_new_product_tags'] ) ? array_map( 'sanitize_text_field', wp_unslash( $_REQUEST['wcj_add_new_product_tags'] ) ) : array(),
				'image'         => isset( $_FILES['wcj_add_new_product_image'] ) ? array_map( 'sanitize_text_field', wp_unslash( $_FILES['wcj_add_new_product_image'] ) ) : '',
				'image_gallery' => isset( $_FILES['wcj_add_new_product_gallery_image'] ) ? $_FILES['wcj_add_new_product_gallery_image'] : '', // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			);
			for ( $i = 1; $i <= $this->the_atts['custom_taxonomies_total']; $i++ ) {
			...
			if ( isset( $_GET['wcj_edit_product_image_delete'] ) ) {
				...
				if ( (string) $user_ID !== (string) $post_author_id ) {
			...
			if ( isset( $_REQUEST['gallary_image_id'] ) ) {
				...
				wp_delete_post( sanitize_text_field( wp_unslash( $_REQUEST['gallary_image_id'] ) ), true );
			...
			if ( 'yes' === $atts['image_gallery_enabled'] ) {
				...
				if ( '' === @$gallary_images ) {  // phpcs:ignore
