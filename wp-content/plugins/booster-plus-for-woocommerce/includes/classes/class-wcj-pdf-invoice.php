<?php
/**
 * Booster Plus for WooCommerce PDF Invoice
 *
 * @version 6.0.4
 * @author  Pluggabl LLC.
 * @package Booster_Plus_For_WooCommerce/classes
 */
...
if ( ! class_exists( 'WCJ_PDF_Invoice' ) ) :
	...
	class WCJ_PDF_Invoice extends WCJ_Invoice {
		 ...
		 * @todo    [dev] maybe `$pdf->SetAuthor( 'Booster Plus for WooCommerce' )`
		...
		public function prepare_pdf() {
			...
			if ( '' !== $background_image ) {
				...
				require_once wcj_plugin_path() . '/includes/classes/class-wcj-tcpdfbg.php';
			} else {
		...
		public function get_html( $order_id, $pdf ) {
			...
			$html = do_shortcode(
				wcj_get_option(
					...
					WCJ()->all_modules['pdf_invoicing_templates']->get_default_template( $this->invoice_type )
				)
			);
		...
		public function get_pdf( $dest ) {
			...
			$styling = '<style>' . wcj_get_option(
				...
				WCJ()->all_modules['pdf_invoicing_styling']->get_default_css_template( $this->invoice_type )
			) . '</style>';
