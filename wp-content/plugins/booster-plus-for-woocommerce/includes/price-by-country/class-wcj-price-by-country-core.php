 ...
 * @version 6.0.5
 * @author  Pluggabl LLC.
 * @package Booster_Plus_For_WooCommerce/includes/Price_By_Country
 */
...
if ( ! class_exists( 'WCJ_Price_By_Country_Core' ) ) :
	...
	class WCJ_Price_By_Country_Core {
		...
		public function handle_wc_points_rewards_settings( $value ) {
			...
			if ( WCJ()->all_modules['currency_exchange_rates']->is_enabled() ) {
				$new_monetary_value = WCJ()->all_modules['currency_exchange_rates']->force_dot_as_exchange_rate_decimal_separator( $new_monetary_value );
			}
		...
		public function enqueue_wselect_scripts() {
			wp_enqueue_style( 'wcj-wSelect-style', wcj_plugin_url() . '/includes/lib/wSelect/wSelect.css', array(), WCJ()->version );
			wp_enqueue_script( 'wcj-wSelect', wcj_plugin_url() . '/includes/lib/wSelect/wSelect.min.js', array(), WCJ()->version, true );
			wp_enqueue_script( 'wcj-wcj-wSelect', wcj_plugin_url() . '/includes/js/wcj-wSelect.js', array(), WCJ()->version, true );
		}
		...
		public function change_price( $price, $product ) {
			...
			if ( null !== $group_id && '' !== $group_id ) {
				...
				if ( $do_save && isset( WCJ()->all_modules['price_by_country']->calculated_products_prices[ wcj_get_product_id( $product ) ][ $_current_filter ] ) ) {
					return WCJ()->all_modules['price_by_country']->calculated_products_prices[ wcj_get_product_id( $product ) ][ $_current_filter ];
				}
				$new_price = wcj_price_by_country( $price, $product, $group_id );
				WCJ()->all_modules['price_by_country']->calculated_products_prices[ wcj_get_product_id( $product ) ][ $_current_filter ] = $new_price;
