	...
	class WCJ_Payment_Gateways_Fees extends WCJ_Module {
		...
		public function check_cart_products( $gateway ) {
			...
			if ( ! empty( $include_products ) ) {
				...
				foreach ( WC()->cart->get_cart() as $item ) {
					if ( in_array( (string) $item['product_id'], $include_products, true ) ) {
						$passed = true;
						break;
					}
				}
			...
			if ( ! empty( $exclude_products ) ) {
				foreach ( WC()->cart->get_cart() as $item ) {
					if ( in_array( (string) $item['product_id'], $exclude_products, true ) ) {
						return false;
					}
				}
			}
		...
		public function gateways_fees() {
			...
			if ( '' !== $current_gateway ) {
				...
				if ( WCJ()->all_modules['multicurrency']->is_enabled() ) {
					$min_cart_amount = WCJ()->all_modules['multicurrency']->change_price( $min_cart_amount, null );
					$max_cart_amount = WCJ()->all_modules['multicurrency']->change_price( $max_cart_amount, null );
				}
				...
				if ( '' !== $fee_text && $total_in_cart >= $min_cart_amount && ( '0' === $max_cart_amount || $total_in_cart <= $max_cart_amount ) && $this->check_cart_products( $current_gateway ) ) {
					...
					switch ( $fee_type ) {
						case 'fixed':
							// Multicurrency (Currency Switcher) module.
							if ( WCJ()->all_modules['multicurrency']->is_enabled() ) {
								$fee_value = WCJ()->all_modules['multicurrency']->change_price( $fee_value, null );
							}
