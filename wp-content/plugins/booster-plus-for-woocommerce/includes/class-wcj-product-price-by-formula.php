...
if ( ! class_exists( 'WCJ_Product_Price_By_Formula' ) ) :
	...
	class WCJ_Product_Price_By_Formula extends WCJ_Module {
		...
		public function save_price( $product_id, $price ) {
			...
			WCJ()->all_modules['product_price_by_formula']->calculated_products_prices[ $product_id ][ $filter ] = $price;
		}
		...
		public function get_saved_price( $product_id ) {
			...
			if ( isset( WCJ()->all_modules['product_price_by_formula']->calculated_products_prices[ $product_id ][ $filter ] ) ) {
				return WCJ()->all_modules['product_price_by_formula']->calculated_products_prices[ $product_id ][ $filter ];
			}
			return false;
		}
		...
		public function change_price( $price, $_product, $output_errors = false ) {
			...
			if ( $this->is_price_by_formula_product( $_product ) && '' !== $price ) {
				...
				$wcj_product_cats_incl   = wcj_get_option( 'wcj_product_cats_incl', '' );
				$wcj_product_cats_option = is_array( $wcj_product_cats_incl ) && count( $wcj_product_cats_incl ) > 0 ? implode( ',', wcj_get_option( 'wcj_product_cats_incl', '' ) ) : '';

				if ( '' !== $wcj_product_cats_option ) {
					if ( 'Include' === wcj_get_option( 'wcj_product_cats_option', '' ) ) {
						...
						$_product_id     = wcj_get_product_id_or_variation_parent_id( $_product );
						if ( in_array( $_product_id, $_product_id_inc, true ) ) {
							$saved_price = $this->get_saved_price( $_product_id );
							if ( false !== $saved_price ) {
								return $saved_price;
							}
							...
							if ( '' !== $the_formula ) {
								...
								if ( $total_params > 0 ) {
									...
									try {
										...
										if ( $output_errors ) {
											echo wp_kses_post( '<p style="color:red;">' . __( 'Error in formula', 'woocommerce-jetpack' ) . ': ' . $e->getMessage() . '</p>' );
										}
									}
									if ( 'no_rounding' !== $this->rounding ) {
		...
		/**
		 * Get_product_id_by_cat_id.
		 *
		 * @version 6.0.0
		 * @since  1.1.6
		 * @param int $category_id defines the category_id.
		 */
		public function get_product_id_by_cat_id( $category_id ) {
