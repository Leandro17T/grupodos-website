	...
	class WCJ_Product_Addons extends WCJ_Module {
		...
		public function maybe_convert_currency( $price, $product = null ) {
			...
			if ( 'by_module' === $apply_price_filters ) {
				...
				if ( ( empty( $modules_to_apply ) || in_array( 'multicurrency_base_price', $modules_to_apply, true ) ) && WCJ()->all_modules['multicurrency_base_price']->is_enabled() ) {
					$price = WCJ()->all_modules['multicurrency_base_price']->change_price( $price, $product );
				}
				// Multicurrency (Currency Switcher) module.
				if ( ( empty( $modules_to_apply ) || in_array( 'multicurrency', $modules_to_apply, true ) ) && WCJ()->all_modules['multicurrency']->is_enabled() ) {
					$price = WCJ()->all_modules['multicurrency']->change_price( $price, $product, array( 'do_save' => false ) );
				}
				// Global Discount module.
				if ( ( empty( $modules_to_apply ) || in_array( 'global_discount', $modules_to_apply, true ) ) && WCJ()->all_modules['global_discount']->is_enabled() ) {
					$price = WCJ()->all_modules['global_discount']->add_global_discount( $price, $product, 'price' );
				}
