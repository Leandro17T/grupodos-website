		...
		if ( function_exists( 'WC' ) && ( WC()->shipping ) && ( $packages ) ) {
			foreach ( $packages as $i => $package ) {
				...
				if ( wcj_is_module_enabled( 'shipping_by_user_role' ) ) {
					$available_shipping_methods = WCJ()->all_modules['shipping_by_user_role']->available_shipping_methods( $available_shipping_methods, false );
				}
				...
				if ( function_exists( 'WC' ) && ( $wc_shipping ) && ( $wc_cart ) ) {
					if ( $wc_shipping->enabled ) {
						...
						if ( $packages ) {
							...
							if ( wcj_is_module_enabled( 'shipping_by_user_role' ) ) {
								$shipping_methods = WCJ()->all_modules['shipping_by_user_role']->available_shipping_methods( $shipping_methods, false );
							}
